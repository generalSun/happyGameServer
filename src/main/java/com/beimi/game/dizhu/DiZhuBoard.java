 ...
 *
 */
public class DiZhuBoard extends Board implements java.io.Serializable{
	...
	public TakeCards takeCardsRequest(GameRoom gameRoom,Board board,Player player,String orgi,boolean auto,byte[] playCards) {
		...
		if((auto == true || playCards != null)){
			...
			if(playCards!=null && playCards.length > 0){
				playCardType = DizhuUtils.getInstance().identification(playCards) ;
			}
			if(playCardType == null || playCardType.getCardtype() > 0){
				if(board.isNewTurn()){	//当前为新的一轮
					...
					if(playCards == null){
						...
						CardType lastCardType = DizhuUtils.getInstance().identification(board.getLast().getCards()) ;
						if(playCardType.getCardtype() >0 && DizhuUtils.getInstance().allow(playCardType, lastCardType)){//合规，允许出牌
							takeCards = board.takecard(player , true , playCards) ;
		...
		if(takeCards!=null){		//通知出牌
			...
			if(board.isWin()){//出完了
				...
				PlayUserClient nextPlayUserClient = EventTools.getInstance().getPlayUserClient(gameRoom.getId(), takeCards.getNextplayer(), orgi) ;
